...
if (isset($_POST['Edit'])) {
    try {
        ...
        if (mysqli_num_rows($checkEmailResult) > 0) {
            header("Location:User.php?msg=L'email a ete Deja exsiste");
        } else {
            
            $sqlEDIT = "UPDATE `user` SET `Nom`='$Nom',`Prenom`='$Prenom',`Email`='$Email' WHERE userID = $id";
            $ResultEDIT = mysqli_query($conn, $sqlEDIT);

            if ($ResultEDIT) {
                header("Location: user.php?msg=L'utilisateur a été modifié avec succès");
            } else {
	...
	<div class="modal-dialog">
		...
		<form method="post" >
				<div class="modal-header">						
					<h4 class="modal-title">La Modification d'utilisateur</h4>
				</div>
				...
				<div class="modal-footer">
				    <a href = "User.php"><input type="button" class="btn btn-default" data-dismiss="modal" value="Cancel"></a>
