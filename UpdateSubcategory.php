...
$sql = "SELECT sub_category.* ,category.CategoryID CategoryID, category.CategoryName CategoryName
FROM sub_category LEFT JOIN category ON
category.categoryID  = sub_category.CategoryID WHERE subcategoryID = $id ";
...
if (isset($_POST['Edit'])) {
    ...
    if($CategoryID != NULL){
      $sqlEDIT = "UPDATE `sub_category` SET `SubCategoryName`='$Nom', `categoryID`='$CategoryID' WHERE subCategoryID=$id";
      $ResultEDIT = mysqli_query($conn, $sqlEDIT);
    }

    else{
      $sqlEDIT = "UPDATE `sub_category` SET `SubCategoryName`='$Nom', `categoryID`= NULL   WHERE subCategoryID=$id";
    $ResultEDIT = mysqli_query($conn, $sqlEDIT);
    }
    ...
    if ($ResultEDIT) {
        header("Location: subCategory.php?msg=L'utilisateur a été modifié avec succès");
    } else {
	...
	<div class="modal-dialog">
		...
		<form method="post" >
				<div class="modal-header">						
					<h4 class="modal-title">La Modification De Sub Category</h4>
				</div>
				<div class="modal-body">					
					<div class="form-group">
						<label>Nom</label>
						<input type="text" name="Nom" class="form-control" value = "<?php echo $row['SubCategoryName'] ?>" required>
					</div>
          ...
          <option value="">Sans Category</option>
				...
				<div class="modal-footer">
				    <a href = "SubCategory.php"><input type="button" class="btn btn-default" data-dismiss="modal" value="Cancel"></a>
